    ...
    <section style="${style}">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
        <div>
          ...
          <h1 style="margin:0;font-size:32px;line-height:1.15;font-weight:800;color:${headingColor};text-transform:uppercase;">${title}</h1>
        </div>
      ...
      <p style="margin:12px 0 0 0;color:${headingColor};opacity:0.9;font-size:14px;">${subtitle}</p>
  ...
  return `
    <section style="${style}">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin:0 0 12px 0;padding-bottom:10px;border-bottom:1px solid #e2e8f0;">
        <h2 style="margin:0;font-size:18px;font-weight:700;color:#1f2937;">${heading}</h2>
...
const renderDescriptionBlock = (block) => {
  ...
  return `
    <section style="${style}">
      <h2 style="margin:0 0 10px 0;font-size:18px;font-weight:700;color:#1f2937;">${title}</h2>
...
const renderSpecsTableBlock = (block) => {
  ...
  const rowHtml = rows
    .map(
      (row) => `
      <tr>
        <td style="padding:10px 12px;border:1px solid #dbe3ef;background:#f8fbff;font-weight:700;color:#0f172a;width:36%;">${
          row.key || ""
        }</td>
        <td style="padding:10px 12px;border:1px solid #dbe3ef;color:#1e293b;">${
          row.value || ""
        }</td>
      </tr>
    `
    ...
    <section style="${style}">
      ...
      <div style="font-size:0;">
        <div style="display:inline-block;vertical-align:top;width:49%;background:#f8fbff;border:1px solid #dbe3ef;border-radius:8px;padding:10px 12px;box-sizing:border-box;margin-right:2%;">
          ...
          <p style="margin:0;font-size:14px;line-height:1.6;color:#1e293b;">${shipping}</p>
        </div>
        <div style="display:inline-block;vertical-align:top;width:49%;background:#f8fbff;border:1px solid #dbe3ef;border-radius:8px;padding:10px 12px;box-sizing:border-box;">
          ...
          <p style="margin:0;font-size:14px;line-height:1.6;color:#1e293b;">${returns}</p>
        </div>
      </div>
    </section>
  `;
